...
- [ ] [p4] [nas-shell] NAS Hop 2 — pg_dump backup then live seed. SHELL: /usr/bin/docker exec apr70v3-postgres-1 pg_dump --format=custom --file=/tmp/apr70-v3-pre-seed-$(date -u +%Y%m%dT%H%M%SZ).dump postgresql://postgres:postgres@localhost:5432/apr70_cms 2>&1 && /usr/bin/docker exec apr70v3-postgres-1 sh -c "ls -lh /tmp/apr70-v3-pre-seed-*.dump" && /usr/bin/docker compose -f /volume1/apps/apr70-pictures/docker-compose.yml -p apr70v3 --profile seed build cms-seeder 2>&1 && /usr/bin/docker compose -f /volume1/apps/apr70-pictures/docker-compose.yml -p apr70v3 --profile seed run --rm --no-deps cms-seeder pnpm exec tsx scripts/migrate-v2-to-v3.ts --apply --v2-root /v2-export/content 2>&1
