...
**Current (2026-09-03, Fable 5.1 — layered-cinema POC CLOSED: three attempts, all killed by Marco; the marquee site stays and is updated in place, mobile first. Keep from the branch: cascade notes, meter fix, shot helper, the Brooklyn line. Earlier note follows.)** **(2026-09-02 ~6:40pm — layered-cinema POC, branch `poc/layered-cinema`, worktree `~/websites/apr70-website/v10-poc`):** the five-layer parallax skin for `/` behind `?design=layered` is BUILT AND VERIFIED, and **NO GO from Marco** (film chrome around the photo, motion unreadable; he wanted depth inside the still: fireworks, woman, ferris wheel as plates). Decision + reshape questions: `docs/decisions/2026-09-02-layered-cinema-poc-no-go.md`. The switch mechanism and island scaffolding survive; the five layers are disposable. Build exit 0, six URLs 200, zero third-party hosts, full screenshot matrix in `docs/handoff/poc-shots/`, `?design=marquee` pixel-identical to `origin/main` (AE 0 on all four baselines). Two defects fixed this session: Tailwind v4's layered `[hidden]!important` beat the un-hide rule so layered mode never rendered (toggle now uses plain CSS display, no `hidden` attr), and the mobile static overlay painted under the absolute img (now `relative` + z-index). Evidence and paths: `docs/handoff/fable-2026-09-02-layered-cinema-poc.md` §9. Side fix: `.claude/hooks/context-meter.sh` no longer bills image payloads (a PDF Read had tripped the gate). Not merged to `main`, no deploy, no version bump, no CMS change.
