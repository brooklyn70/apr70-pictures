...
- [x] [p11] Layered-cinema POC: CLOSED 2026-09-03 after three attempts (marquee chrome, pinned depth sketch, true-parallax recreation), all killed by Marco. Marquee site stays. Record: `docs/decisions/2026-09-02-layered-cinema-poc-no-go.md`.
- [x] [p11] **v14 SHIPPED 2026-09-03** (mobile pass + brand housekeeping + Brooklyn fold): `SITE_VERSION` v14, ledger row, vault note, handoff archived.
- [x] [p11] [claude] **Mobile pass on the marquee site** (2026-09-03, branch `fix/mobile-pass`): gallery srcset, hero tier + cover sizes, 4:5 phone folds, header hide-on-scroll + 44px targets, 13px mono floor, quicker phone splash, font subset. Numbers in BRIEF.
- [x] [p11] [claude] **Mobile tap-target pass** (2026-09-04, PR #8 `fix/mobile-targets`): 44px controls past the header on phones (filmstrip arrows/dots, scroll-top, footer routes, route line, wordmarks, contact fields), 13px floor on the filmstrip arrows. Audit at 375x812 @2x on six routes: 0 targets under 44px, 0 text under 13px. Stale parallax PNGs deleted from `_sketches/parallax-2026-09-03/`; leftover local servers (4321/4333/4399/4400) killed.
- [x] [p11] [claude] **Hero lines** (2026-09-04, PR #10 `feat/hero-lines`): Marco moved the Brooklyn line off the home page; nine one-liners now sit over each project hero via `Project.heroLine` (migration, overlay in `/work/[slug]`, seed maps canon `h1:`, SQL on the NAS, canon files updated, `index.md` fold removed). Lines listed in BRIEF; Marco to read and edit in Payload, mirror edits to the canon.
- [ ] [p11] [requires-gui] **Hero lines ballot** (2026-09-04): Marco ruled the live lines lean on the images; five story-only candidates per property are in the vault at `11.03 Company Ops/00 Website - Hero Lines Ballot.md`. Marco ticks one per property (or writes in), then says "apply the ballot": Payload → Hero line, canon `h1:`, staging, screenshots.
- [x] [p11] [claude] DONE 2026-09-03 (live NAS DB via SQL insert as home section 9 before "On the page now."; copy canon `11.12 V9 Build/02-copy/index.md` in the SharedData vault archive; seed-v9 now falls back to that archive path). **Copy: "Brooklyn, before it was a brand."** on the existing site for NRC / U Bruculinu (Payload edit; rule 16: live DB + seed + vault copy canon). Marco 2026-09-02: "I love Brooklyn before it was a brand. keep that on the existing site."
- [ ] [p11] [claude] (superseded) Layered-cinema POC on `/` (plan §C): five Astro layers, one GSAP ScrollTrigger island on `client:visible`, `themes/layered.css` scoped under `html[data-design="layered"]`, `?design=layered` flag + Site Settings `layeredDesignPreview` switch (OFF by default), `layeredFold` section fed by Payload media, PD ledger plate in the gate. Reduced-motion static, both modes, 375/1440 screenshots, six-URL check. Bump `SITE_VERSION` to `v14`, ledger row + shots.
...
- [x] [p11] [claude] **i18n groundwork — CMS side DONE 2026-09-07** (branch `i18n/groundwork`, plan §F.2 phase 2). Payload `localization` (en default, pt/it/fr/de, `fallback: true`) reading `cms/src/locales.ts`; 195 text fields marked `localized: true` across `v9Sections`, `v9Pages`, `SiteSettings.v9Chrome`, `Project`, `Media.alt` and the legacy blocks/collections (inventory: `docs/i18n/localized-fields.md`); stored AI Mark flag `Media.aiFrame` + idempotent backfill (`pnpm aimark:apply`, flagged 39 of 206); `SiteSettings.enabledLocales` (English not removable); `seed-v9.ts --locale=<code>`; migration `20260907_195421_i18n_localization` rehearsed on a fresh NAS-dump copy with zero content lost in both directions (`docs/i18n/migration-rehearsal-2026-09-07.md`); `preflight` green. **Web side DONE 2026-09-07** (same branch, plan §F.2 phase 1 + the Phase 3 routing/switcher parts, gated so nothing visible changes until a locale is enabled): `web/src/middleware.ts` reads a locale prefix and rewrites to the unprefixed path (Astro `i18n.routing: 'manual'`, pages still live once under `src/pages/`); English unprefixed, others `/pt/...`; a locale absent from `SiteSettings.enabledLocales` 404s through the site's existing 404; `locale` threaded through every fetcher in `lib/payload.ts` with `:${locale}` on every cache key and NO `&locale=` on English URLs; `localePath()` on every internal link; `<html lang>`, per-locale canonical, hreflang for enabled locales + `x-default`, `og:locale`, `inLanguage`, sitemap `xhtml:link` alternates, one `Languages` line in `llms.txt`; zero-JS mono-keycode `LanguageSwitcher.astro` mounted in the header nav and footer nav row, rendering nothing while one locale is enabled; stored `Media.aiFrame` now authoritative in `media.ts` and `FilmstripSlideshow.tsx` with the English regex kept as fallback; non-CMS English strings routed through `lib/i18n/dictionary.ts` (`en` populated, other locales empty and falling back — no invented translations). Verified against the rehearsal DB: `docs/i18n/web-verification-2026-09-07.md` (English HTML diffs from the pre-i18n build ONLY by hreflang/`og:locale`/`inLanguage` + asset hashes; AI-mark stamp counts identical; `astro check` unchanged at its 8 pre-existing errors). Dev-only override `I18N_FORCE_ENABLED` must never be set in production. **Still open:** the vault copy canon per locale and the translated copy itself. RULED 2026-09-02: go-live ships English first, languages after; first translation round by Claude (VMS precedent), Marco signs off each locale before it ships. Translated copy canonised at `11.12 V9 Build/02-copy/<locale>/` only after sign-off.
