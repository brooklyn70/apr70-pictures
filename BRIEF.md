# BRIEF — apr70-pictures (v14 on the NAS: staging.apr70.com = full site, apr70.com = holding page)

**Current (2026-09-04 later, Fable 5.1 — hero lines on the project pages, home fold retired, PR #10, DEPLOYED to staging, main 8e6912d):** Marco's ruling: "Brooklyn, before it was a brand." comes OFF the home page (it would be the only fold with words there) and every property gets its own one-liner over the hero on its project page only. Built: `Project.heroLine` (text, nullable) + additive migration `20260904_070000_hero_line` (ran on the NAS at deploy, 87ms); `/work/[slug]` renders it as a `<p class="v9-photofold__heading">` in the photo-fold overlay (light ink on the scrim, both modes, 375 and 1440 verified by screenshot); `seed-v9` maps the property fold's `h1:`. Three layers landed (rule 16): NAS DB via SQL (nine `hero_line` rows, the home fold row deleted, sections 10+ shifted back to 9+), the copy canon (`h1:` in nine `properties/*.md`, the fold block removed from `index.md`), seed reads the canon. The nine lines: ANGIB "Brooklyn, before it was a brand." / L.A. Dolce Vita "Two cities, ten days, one scandal." / Sea Gate "A private town, one road in." / Alpha YY "Staten Island to Little Italy, collecting." / Da Hook "The piers empty, the towers rise." / The Movement "The city is the campus." / Cleopatra "A new life every morning, same address." / Shadowmaster "The player has the plan. The body has a conscience." / U Bruculinu "Home to a village that never knew him." The Mayors has no hero and no line. Editable in Payload (Projects → Hero line); any edit must be mirrored to the canon file. Home verified: only the pier hero carries a heading.

**Current (2026-09-04, Fable 5.1 — mobile tap-target pass, PR #8 `fix/mobile-targets`, DEPLOYED to staging from `release/v14` = `main` 59fe8b1):** session cleanup first: the four leftover local servers (4321 dev, 4333 preview, 4399/4400 dead-sketch http.servers) killed; the 11 parallax PNGs (depth plates + reference shots) deleted from `_sketches/parallax-2026-09-03/` (11MB of html/mp4/jpg/vendor js remains, dead, one `rm -rf` away). Then a tap-target audit at 375x812 @2x on staging v14 found what the mobile pass missed below the header: filmstrip arrows 35px at 12.8px type, sprocket dots 9px, scroll-top 38px, footer routes 24px tall, route line 26px, "more"/ring-return 26px, wordmarks 40/26px, contact fields 40px. All phone-only CSS in `v9.css`, `filmstrip-slideshow.css`, `roll-form.css`: padding + negative margin where there is no rule under the text, a `::before` hit area where there is (footer routes, "more", ring return), the dots on a 44px pitch with their size unchanged, scroll-top 44px (rule lives in the craftrail phone block because the base rule is later in the file). Verified on the public host after deploy, six routes: 0 targets under 44px (the two contact flags are the off-screen honeypot and the checkbox inside its 44px label), 0 text under 13px, no horizontal overflow, no page errors. Preflight passed. Open, still Marco's call: the Brooklyn line on the A Need Grows in Brooklyn project page.

**Current (2026-09-03, Fable 5.1 — mobile pass on the marquee site, branch `fix/mobile-pass` off main):** the layered-cinema POC is CLOSED (three attempts, all killed by Marco; decision doc `docs/decisions/2026-09-02-layered-cinema-poc-no-go.md`). The marquee site stays and is improved in place. Seven mobile fixes, measured at 375x812 @2x on the production build: work-page galleries now carry srcset (L.A. Dolce Vita 2.7MB → 1.0MB), heroes get the original as a srcset tier when no 1920 tier exists and the fold's `sizes` asks for the cover crop (hero upscale 3.2x → 1.7x), non-hero folds are 4:5 on phones instead of a 156px band, the sticky header hides on scroll-down and returns on scroll-up with 44px tap targets, the letterspaced mono floor is 13px on phones (0 elements under 13px, was 35 to 43 per page), the splash runs at half length on coarse pointers (2.5s → 1.5s), fonts re-subset (already Latin, only 12KB saved; the earlier "halve it" estimate was wrong). Kept from the POC branch: cascade-trap notes, context-meter fix, screenshot helper. Copy line "Brooklyn, before it was a brand." is LIVE on staging as a home photo fold (hero-portrait-v2, before "On the page now."), written to all three layers: NAS DB (SQL, section _order 9, later blocks shifted), the copy canon (`index.md` in the SharedData vault archive, `h1:` key), and seed-v9 (which now finds the canon at the archive path; the live vault no longer has 11.12 V9 Build). **v14 SHIPPED 2026-09-03 on Marco's go:** `SITE_VERSION` v14; ledger row + refreshed shots in MASTER-RECAP; vault version note updated; handoff archived per rule 15. **DEPLOYED to the NAS stack (staging.apr70.com) 2026-09-03 ~11:40 from `fix/mobile-pass` (87e80a0) and merged to `main`; verified on the public host at 375x812: six routes 200, 0 text under 13px, nav 45px / toggle 44px, folds 4:5, gallery on the 1024 tier, header hides on scroll. Deploy lesson: the NAS Docker daemon runs with iptables off, so build containers have no DNS; `docker-compose.yml` now builds on the host network (`build.network: host`). The deploy ran from the worktree with a scratch copy of the deploy script whose local-DB checks are skipped for code-only runs; the canonical `_deploy/deploy-v10-to-nas.sh` still requires the local Postgres for its preflight.**

**Current (2026-09-07, Fable 5.1 — i18n groundwork + Portuguese draft, branch `i18n/groundwork`, NOT deployed):** Payload localization en/pt/it/fr/de with a data-preserving migration rehearsed on a fresh NAS dump; Astro locale routing (`/pt/…`), hreflang/sitemap/inLanguage, mono-keycode language switcher gated by Site Settings → Languages → Enabled locales (default English only, so nothing public changes); stored AI Mark flag on Media; Portuguese first draft (618 fields, claude-opus-5) applied to a local rehearsal DB with a sign-off doc at `docs/i18n/review/pt-2026-09-07.md`. Handoff: `docs/handoff/fable-2026-09-07-i18n-groundwork-pt.md`. Open calls: `pt` vs `pt-BR`, switcher placement, PT sign-off. Same session, separate deliverable: writers-room copy pass of the live English copy into Marco's voice + critic panel (draft only, Word doc on SharedData).
...
Plan §A.2 stale-header corrections APPLIED on Marco's go (this header, CLAUDE.md hosting lines, umbrella README, deploy-script final line, Vercel runbook status, RECAP v13 note). Vercel/Supabase are not the production path; the V4 block below is history. **Marco's rulings recorded in plan §G.1 and queued as TASKS Phase 11:** POC on `/` behind `?design=layered` (build now with PD plates), Punch kit canonical, brand dir archives to the NAS, restore tag on `6a6fa4a`, next bump is `v14`, Stop hook to skip subagent stops, rights-status label on every external reference. Ruled later the same day: go-live ships English first; Claude does the first translation round with Marco signing off each locale. **Done same day:** v13 restore point (tag `restore/v13-2026-09-02` on `6a6fa4a` + NAS dump + media snapshot), GitHub branch protection on `main`, hooks rewritten (per-session context meter; stop hook commits tracked files only, no BRIEF auto-notes), Crop Studio fixes (launcher PATH, arrows from the roll, Archive-to-90-archive), brand housekeeping (Punch fallbacks, brand tree + Futura-named SVGs archived to the NAS and removed, media 209 deleted, `seed-brand-kit.ts`). POC handoff for the parallel session: `docs/handoff/fable-2026-09-02-layered-cinema-poc.md` (worktree `poc/layered-cinema`). Housekeeping is on `main` but NOT deployed: it ships as `v14` with the POC after preflight and Marco's go.
...
**Historical (July V4 Vercel experiment, not the production path; DNS never moved):** **Phase:** V4 SHIPPED TO VERCEL STAGING — https://apr70-web.vercel.app (Astro serverless) + https://apr70-cms-brooklyn70-brooklyn70s-projects.vercel.app (Payload → Supabase `apr70` ref rrxeqsryndjoivcsnkqq). NAS still serves apr70.com, untouched; DNS cutover deliberately NOT done.
...
## Auto-stop note (2026-09-02 18:47 UTC)

- Branch: main
- Tip: 0855c47

## Auto-stop note (2026-09-02 18:49 UTC)

- Branch: main
- Tip: 942a307

## Auto-stop note (2026-09-02 18:59 UTC)

- Branch: main
- Tip: 8da6491

## Auto-stop note (2026-09-02 19:04 UTC)

- Branch: main
- Tip: 49f6ec6

## Auto-stop note (2026-09-02 19:16 UTC)

- Branch: main
- Tip: 2ac6705

## Auto-stop note (2026-09-02 19:32 UTC)

- Branch: main
- Tip: e123437

## Auto-stop note (2026-09-02 20:02 UTC)

- Branch: main
- Tip: c083271

## Auto-stop note (2026-09-02 20:19 UTC)
