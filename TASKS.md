...
- [x] [p11] [claude] **Layered-cinema POC on `/`** BUILT AND VERIFIED on `poc/layered-cinema` (0ab0662), then **NO GO from Marco 2026-09-02**: film-chrome around the photo, motion too small to read. See `docs/decisions/2026-09-02-layered-cinema-poc-no-go.md`. Not merged, no v14 bump from this.
- [x] [p11] Reshape questions answered by Marco 2026-09-02 (reference aiautomationsociety.ai; depth inside the photo; plates cut by the agent; no chrome; whole page). A pinned/scaling depth sketch was shown and KILLED the same night: wrong reading of parallax. See the decision addendum.
- [ ] [p11] [claude] **Keep the line:** put "Brooklyn, before it was a brand." on the existing site for NRC / U Bruculinu (Payload copy edit, three layers per rule 16). Marco: "I love Brooklyn before it was a brand. keep that on the existing site."
- [x] [p11] KILLED 2026-09-03 (Marco: stick with the current version). Study + recreation: layers translate at different rates as the page scrolls, no pin, no zoom. Research filed in docs/handoff; eight reference sites re-studied; recreate the useful example as an original build. Marco sets the goal 2026-09-03 morning.
- [ ] [p11] [claude] (superseded spec) Layered-cinema POC on `/` (plan §C): five Astro layers, one GSAP ScrollTrigger island on `client:visible`, `themes/layered.css` scoped under `html[data-design="layered"]`, `?design=layered` flag + Site Settings `layeredDesignPreview` switch (OFF by default), `layeredFold` section fed by Payload media, PD ledger plate in the gate. Reduced-motion static, both modes, 375/1440 screenshots, six-URL check. Bump `SITE_VERSION` to `v14`, ledger row + shots.
