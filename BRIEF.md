...
**Current (2026-09-02 ~5:40pm, Fable 5.1 — layered-cinema POC, branch `poc/layered-cinema`, worktree `~/websites/apr70-website/v10-poc`):** the five-layer parallax skin for `/` behind `?design=layered` is WRITTEN AND COMMITTED BUT NOT BUILT OR VERIFIED. The rule-14 context gate blocked the session after the source landed and before `pnpm build` or a single layered screenshot. Files: `web/src/styles/themes/layered.css` (all rules scoped under `html[data-design="layered"]`), the pre-paint stamp in `V9Layout.astro`, the host wrapper in `PhotoFold.astro`, five Astro layer components under `web/src/components/v9/layered/`, and `LayeredParallaxIsland.tsx`. Baseline marquee screenshots of `origin/main` (dark/light x 1440/375, real NAS media) are in `docs/handoff/poc-shots/`. Pickup instructions, known risks, and the untouched definition-of-done are in `docs/handoff/fable-2026-09-02-layered-cinema-poc.md` §8. Not merged to `main`, no deploy, no version bump, no CMS change.
